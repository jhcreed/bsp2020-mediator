...
---
background-image: url(figures/causal_title.png)
background-size: 105%
...
## Is the effect of being uninsured on overall survival mediated through treatment delays in prostate cancer ...

Based on > 500,000 prostate cancer cases in the National Cancer Data Base
+ A greater proportion of those who are uninsured experience a 6 month treatment delay than those who are insured
+ Being uninsured is associated with worse outcomes in prostate cancer
+ A 6 month treatment delay is associated with worse overall survival in prostate cancer than those who receive more timely treatment
...
dat %>% 
  ...
  mediator(data = dat_clean,
                   out.model = coxph(Surv(DX_LASTCONTACT_DEATH_MONTHS, death) ~
                                       uninsured + delay_6months + 
                                       PSA + cGleasonSum + AGE + racev2 + 
                                       (uninsured*delay_6months), 
                                     data = dat_clean),
                   med.model = glm(delay_6months ~ uninsured +
                                     PSA + cGleasonSum + AGE + racev2, 
...
load("/Volumes/usersC$/NCDB/ACSgrant/gleasonUpgrading/data/raw.Rdata")
...
dat_clean <- dat %>%
  ...
  mutate(delay_6months = case_when(
    delay_months >= 6 ~ 1,
    delay_months < 6 ~ 0,
    TRUE ~ NA_real_
  ))
...
+ CDE of being uninsured compared to insured when forcing treatment to occurr within 6 months is 1.95
+ CDE of being uninsured compared to insured when forcing treatment to occurr after 6 months is 1.57
...
NDE:  the effect of the exposure (uninsured) on the outcome (overall survival) if the pathway from the exposure to the mediator (delays in treatment) was removed
+ HR for beinf uninsured compared to insured, when each individuals level of delay is kept at the level it would take in the absence of insurance status, is 1.93 (1.81-2.07)
...
NIE: the effect of the exposure (uninsured) that operates by changing the mediator (treatment delays)
+ HR for being uninsured, if TTT was changed from the level it would take if insurance status was insured to the level if insurance status was uninsured, is 1.00 (0.99-1.00)
...
TE:  the overall effect of the exposure (uninsured) on the outcome (overall survival)
+ HR for uninsured compared to insured, overall, is 1.93 (1.80-2.06)
