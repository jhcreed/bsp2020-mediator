...
## Is the effect of being uninsured on overall survival mediated through time to treatment in prostate cancer ...

Based on > 500,000 prostate cancer cases in the National Cancer Data Base
+ Those who are uninsured have a longer average TTT than those who are insured
+ Being uninsured is associated with worse outcomes in prostate cancer
+ Longer TTT is protective towards overall survival in prostate cancer
...
dat %>% 
  ...
  mediator(out.model = coxph(Surv(DX_LASTCONTACT_DEATH_MONTHS, death) ~ 
                               uninsured + delay_months +  PSA + cGleasonSum + AGE + racev2 + 
                               (uninsured*delay_months), 
                             data = .),
                   med.model = glm(delay_months ~ uninsured + PSA + cGleasonSum + AGE + racev2, 
                                   data = dat_clean),
                   treat = "uninsured")
```
...
mediator(data = dat_clean,
                   out.model = coxph(Surv(DX_LASTCONTACT_DEATH_MONTHS, death) ~
                                       ...
                                       (uninsured*delay_months), 
                                     data = dat_clean),
                   med.model = glm(delay_months ~ uninsured +
                                     PSA + cGleasonSum + AGE + racev2, 
                                   data = dat_clean),
                   treat = "uninsured") %>%
...
+ CDE of being uninsured compared to insured when forcing TTT as 0 months is 1.97
+ CDE of being uninsured compared to insured when forcing TTT as 6 months is 1.90
...
NDE:  the effect of the exposure (uninsured) on the outcome (overall survival) if the pathway from the exposure to the mediator (time to treatment) was removed
+ HR for beinf uninsured compared to insured, when each individuals level of TTT is kept at the level it would take in the absence of insurance status, is 1.94 (1.81-2.08)
...
NIE: the effect of the exposure (uninsured) that operates by changing the mediator (time to treatment)
+ HR for being uninsured, if TTT was changed from the level it would take if insurance status was insured to the level if insurance status was uninsured, is 0.99 (0.98-1.00)
...
+ HR for uninsured compared to insured, overall, is 1.92 (1.79-2.05)
